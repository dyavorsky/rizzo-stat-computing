...
Use the inverse transform method. First recall the cdf $F_X(x)$ and find $F_X^{-1}(u)$. 
$$
u = F_X(x) = 1 - e^{-\lambda(x-\eta)} 
\hspace{1em} \rightarrow \hspace{1em}
F_X^{-1}(u) = -\log(1-u)/\lambda + \eta
$$

Next define a function to generate random variates of $X$ using $F_X^{-1}(u)$.

```{r}
rexp2 <- function(n, lambda, eta) {
   -1*log(1 - runif(n))/lambda + eta
}
```

Calculate the common empirical quantiles.
...
```{r}
rexp2(n=1e4, lambda=3, eta=2) |> summary()
```

Recall that for quantile $\alpha$, $F(x_\alpha)=\alpha$ with $x_\alpha = -\log(1-\alpha)/\lambda + \eta$. Compare the empirical quantiles from our large sample to the theoretical quantiles.

\begin{align*}
   x_{0.25} &= -\log(1-0.25)/3+2 = `r (-log(1-0.25)/3+2) |> round(3)` \\
   x_{0.50} &= -\log(1-0.50)/3+2 = `r (-log(1-0.50)/3+2) |> round(3)` \\
   x_{0.75} &= -\log(1-0.75)/3+2 = `r (-log(1-0.75)/3+2) |> round(3)`
\end{align*}
...
First find the cdf and its inverse.

$$
\begin{equation}
  F_X^{-1}(u)  =
    \begin{cases}
      \exp\{x\}/2 & \text{if } x \le 0\\
      1 - \exp\{-x\}/2 & \text{if } x > 0
    \end{cases}       
\end{equation}
$$
$$
F_X^{-1}(u) = -\text{sign}(u-0.5) \times \log(1 - 2 \times |u-0.5|)
$$
...
set.seed(1234)
x <- rlaplace(1e4)
s <- seq(from=-10, to=10, by=0.1)
y <- exp(-abs(s))/2

ggplot() + 
   geom_histogram(data=tibble(x), aes(x, y=after_stat(density)), bins=100) +
   geom_line(data=tibble(s,y), aes(x=s, y=y), linewidth=1, col="firebrick") + 
...
```{r}
rpareto <- function(n, a, b) b*(1-runif(n))^(-(1/a))
```
...
set.seed(1234)
x <- rpareto(1e4, 2, 2)
s <- seq(from=2, to=15, by=0.1)
dpareto <- function(x, a, b) a * b^a / x^(a+1)
y <- dpareto(s, 2, 2)

ggplot() + 
   geom_histogram(data=tibble(x), aes(x, y=after_stat(density)), bins=100) +
   geom_line(data=tibble(s,y), aes(x=s, y=y), linewidth=1, col="firebrick") + 
   ggtitle("Pareto(2,2) Distribution") + 
   labs(x="", y="") + 
   xlim(0,15) +
   theme_minimal()
```
...
Use the inverse transform methos. First find $F(x) = \int f(x) dx = 1 - e^{-x^2/(2\sigma^2)}$ so that $F^{-1}(u)^2 = -2\sigma^2\log(1-u)$. 

```{r}
rrayleigh <- function(n, sigma) sqrt(-2*sigma^2*log(runif(n)))
```
...
ggplot() + 
   geom_histogram(data=tibble(x1), aes(x1, y=after_stat(density)), bins=100) +
   geom_line(data=tibble(s,y1), aes(x=s, y=y1), linewidth=1, col="firebrick") + 
   ggtitle("Rayleigh(2) Distribution") + 
   labs(x="", y="") + 
   xlim(0,15) +
   theme_minimal()
...
ggplot() + 
   geom_histogram(data=tibble(x2), aes(x2, y=after_stat(density)), bins=100) +
   geom_line(data=tibble(s,y2), aes(x=s, y=y2), linewidth=1, col="firebrick") + 
   ggtitle("Rayleigh(4) Distribution") + 
   labs(x="", y="") + 
   xlim(0,15) +
   theme_minimal()
...
u <- runif(n)
cp <- cumsum(c(0.1, 0.2, 0.2, 0.2, 0.3))
x1 <- (u>cp[1]) + (u>cp[2]) + (u>cp[3]) + (u>cp[4]) + (u>cp[5])

# sample method
x2 <- sample(x=0:4, size=n, replace=T, prob=c(1,2,2,2,3)/10)

# tabulate both methods
merge(
   tibble(x=x1) |> group_by(x) |> summarize(n1=n()) |> mutate(perc1=n1/sum(n1)),
   tibble(x=x2) |> group_by(x) |> summarize(n2=n()) |> mutate(perc2=n2/sum(n2)),
   by="x"
)
```
...
(This is an alternative parameterization of the Paredo cdf given in Exercise 3.3.) Generate 1000 random observations from the mixture with $r=4$ and $\beta = 2$. Compare the empirical and theoretical (Pareto) distributions by graphing the density histogram of the sample and superimposing the Pareto density curve.
