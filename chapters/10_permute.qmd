...
Note that $U$ can be vectorized and evaluated in one line of R code. Then te cramer-von Mises two-sample statistic is
$$
...
The `iris` data is a four-dimensional distribution with measurement on three species of iris flowers. Using your function `Tnr` of Exercise 10.4 and the `boot` function, applyl your nearest neighbors statistic ($r=2$) to test $H_0: F=G$ where $F$ is the distribution of the iris _setosa_ species, ahnd $G$ is the distribution of the iris _virginica_ species. Repeat the test with $r=3$ and $r=4$.
